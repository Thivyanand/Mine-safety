...
def main():
    ...
    # Sensor Digital Twin runs independently of the worker simulator — it
    # works whether workers come from the built-in simulator or a real
    # watch/telemetry POST, and drives the gas/oxygen/temperature/vibration/
    # structural sensors plus the UWB anchors. All values are SIMULATED
    # SENSOR TELEMETRY.
    sensor_thread = threading.Thread(target=dashboard_base.sensor_loop, daemon=True)
    sensor_thread.start()
